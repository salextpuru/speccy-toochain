...
int parse_arg(int argc, char* argv[], const char* arg, int n)
{
...
void parse_args(int argc, char* argv[])
{
	...
	if (i = parse_arg(argc, argv, "-b", 2)) {
		...
		if (i = parse_arg(argc, argv, "-c", 1))
			...
			else if( !strcmp("mlz",argv[i]) ){
				conf.packer = PM_MLZ;
			}
			else if( !strcmp("hst",argv[i]) ){
				conf.packer = PM_HST;
			}
			else {
...
void pack_blocks()
{
	...
	for (int i=0; i<conf.n_blocks; i++) {
		...
		if (conf.packer == PM_AUTO || conf.packer == PM_MLZ) {
			std::string cmd("mhmt -mlz ");
			cmd=cmd+f1n+" "+f2n;

			if (system(cmd.c_str()) <0) error(MHMT);
